<script>
    ...
    async function studentCreate(e) {
        e.preventDefault();

        const response = await studentPost(student);
        const responseBody = await response.json();

        if (responseBody.statusCode === 200) {
            await alertSuccess('create student successfully');
            await goto('/student');
        } else {
...
<section class="p-6 bg-white rounded-2xl shadow-md mx-auto">
    <h2 class="text-2xl font-bold mb-6 text-gray-700">Student Add</h2>
